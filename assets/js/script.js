...
function renderWeatherData(data) {
    ...
    function formatDate(dateString) {
      return dayjs(dateString).format('DD/M/YYYY');
    }
  
    function formatDateHours(dateString) {
      return dayjs(dateString).format('DD/M/YYYY hh:mm:ss');
    }

    const cityName = data.city.name;
    const currentDate = formatDate(data.list[0].dt_txt);
    ...
    const fiveDaysSection = $('#5-days');
  
    forecastSection.empty(); 
    fiveDaysSection.prepend(`
        <h3>5-Day Forecast:<h3>
        `);
  
    for (let i = 1; i < data.list.length; i++) {
      const forecastDate = formatDateHours(data.list[i].dt_txt);
